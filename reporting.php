<?php
	...
	function residentStats(){
		totalCompletedEvaluationsResident();
		echo "<br /><br />";
		residentsWithoutEvals();
		echo "<br /><br />";
		lastCompletedEvaluationsResident();
	}

	function residentsWithoutEvals(){
		global $mysqli;

		$residents = $mysqli->query("select username from users where type='resident'");
		$requests = $mysqli->query("select resident from requests where status='pending' or status='complete' or status='disabled'");
		$evaluations = $mysqli->query("select resident from requests where status='complete'");
		...
		echo "<h3>No evaluations completed</h3>";
		echo "<table class='table'><tr>";
		$i = 0;
	...
	function lastCompletedEvaluationsResident(){
		global $mysqli;

		$requests = $mysqli->query("select resident, completeDate from requests where status='complete'");
	...
	function totalCompletedEvaluationsResident(){
		global $mysqli;

		$requests = $mysqli->query("select resident, status from requests");
		...
		echo "<th>Completed</th>";
		foreach($allResidents as $resident){
			echo "<td>{$numCompletedEvals[$resident]}</td>";
		}
		echo "</tr>";
		...
		echo "<th>Requests</th>";
		foreach($allResidents as $resident){
			echo "<td>{$numRequests[$resident]}</td>";
		}
		echo "</tr>";
