<?php
	...
	function email_new_ruser($user,$pass,$first,$last,$email_address)
        {
		...
		$message.= "If you run into any difficulties or problems, please email me directly: kjenner@mcw.edu and I will respond as soon as possible.\n\n";
	...
	if($accountType == "resident"){
		if($stmt = $mysqli->prepare("insert into users (username, password, email, firstName, lastName, type, status, createdDate, trainingLevel, photo) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);")){
			if($stmt->bind_param("ssssssssss", $username, $password, $email, $firstName, $lastName, $accountType, $status, $evaluationDate, $trainingLevel, $photoPath))
				if($stmt->execute()){
					$success = "true";
					email_new_ruser($username,$original_password,$firstName,$lastName,$email);
				}
				else{
					print $stmt->error;
				}
	...
	header("Location: manage_accounts.php?add={$success}");
?>
