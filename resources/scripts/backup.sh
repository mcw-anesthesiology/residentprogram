...
tar -cz $FORM_DIRECTORY | curl -1 --disable-epsv --ftp-skip-pasv-ip -u $BOX_USERNAME:$BOX_EXTERNAL_PASSWORD --upload-file /dev/stdin ftp://ftp.box.com/$BOX_DIRECTORY/$FORM_DIRECTORY/$BACKUP_FORM_FILENAME
tar -cz $PHOTO_DIRECTORY | curl -1 --disable-epsv --ftp-skip-pasv-ip -u $BOX_USERNAME:$BOX_EXTERNAL_PASSWORD --upload-file /dev/stdin ftp://ftp.box.com/$BOX_DIRECTORY/$PHOTO_DIRECTORY/$BACKUP_PHOTO_FILENAME
