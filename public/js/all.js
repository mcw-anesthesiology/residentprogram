...
!(function (e, n, t) {
	function r(e, n) {
		...
		var s,
		    ...
		    u = "modernizr",
		    d = i("div"),
		    c = p();if (parseInt(r, 10)) for (; r--;) l = i("div"), l.id = o ? o[r] : u + (r + 1), d.appendChild(l);return s = i("style"), s.type = "text/css", s.id = "s" + u, (c.fake ? c : d).appendChild(s), c.appendChild(d), s.styleSheet ? s.styleSheet.cssText = e : s.appendChild(n.createTextNode(e)), d.id = u, c.fake && (c.style.background = "", c.style.overflow = "hidden", f = w.style.overflow, w.style.overflow = "hidden", w.appendChild(c)), a = t(d, e), c.fake ? (c.parentNode.removeChild(c), w.style.overflow = f, w.offsetHeight) : d.parentNode.removeChild(d), !!a;
	}function m(n, r) {
		...
		}for (var d, p, c, v, h, y = ["modernizr", "tspan"]; !T.style;) d = !0, T.modElem = i(y.shift()), T.style = T.modElem.style;for (c = e.length, p = 0; c > p; p++) if ((v = e[p], h = T.style[v], a(v, "-") && (v = l(v)), T.style[v] !== t)) {
			if (s || r(o, "undefined")) return f(), "pfx" == n ? v : !0;try {
				T.style[v] = o;
			} catch (g) {}if (T.style[v] != h) return f(), "pfx" == n ? v : !0;
		}return f(), !1;
	    ...
	    C = [],
	    x = { _version: "3.2.0", _config: { classPrefix: "", enableClasses: !0, enableJSClass: !0, usePrefixes: !0 }, _q: [], on: function on(e, n) {
	    ...
	    Modernizr = function Modernizr() {};Modernizr.prototype = x, Modernizr = new Modernizr();var w = n.documentElement,
	    _ = "svg" === w.nodeName.toLowerCase();Modernizr.addTest("canvas", function () {
		var e = i("canvas");return !(!e.getContext || !e.getContext("2d"));
	});var S = "Moz O ms Webkit",
	    b = x._config.usePrefixes ? S.split(" ") : [];x._cssomPrefixes = b;var E = x._config.usePrefixes ? S.toLowerCase().split(" ") : [];x._domPrefixes = E;var P = { elem: i("modernizr") };Modernizr._q.push(function () {
		delete P.elem;
	});var T = { style: P.elem.style };Modernizr._q.unshift(function () {
		delete T.style;
	}), x.testAllProps = h, x.testAllProps = y, Modernizr.addTest("flexbox", y("flexBasis", "1px", !0)), Modernizr.addTest("flexwrap", y("flexWrap", "wrap", !0)), o(), s(g), delete x.addTest, delete x.addAsyncTest;for (var z = 0; z < Modernizr._q.length; z++) Modernizr._q[z]();e.Modernizr = Modernizr;
})(window, document);
// Closure
(function () {
	/**
...
function reportHtml(i) {
	return '<div class="report-options collapse">' + '<button type="button" class="close remove-report-group" aria-hidden="true">&times;</button>' + '<h3>Report</h3>' + '<div class="form-group">' + '<label for="startDate' + i + '">Start Date:</label>' + '<input type="text" class="form-control datepicker startDate" id="startDate' + i + '" name="startDate' + i + '">' + '</div>' + '<div class="form-group">' + '<label for="endDate">End Date:</label>' + '<input type="text" class="form-control datepicker endDate" id="endDate' + i + '" name="endDate' + i + '">' + '</div>' + '<div class="form-group" style="text-align: center;">' + '<button type="button" class="btn lastThreeMonths">Last Three Months</button> ' + '<button type="button" class="btn lastSixMonths">Last Six Months</button>' + '</div>' + '<div class="form-group">' + '<label for="trainingLevelInput' + i + '">Training Level</label>' + '<select class="form-control select2" id="trainingLevelInput' + i + '" name="trainingLevel' + i + '" style="width: 100%" required>' + '<option value="all">All</option>' + '<option value="intern">Intern</option>' + '<option value="ca-1">CA-1</option>' + '<option value="ca-2">CA-2</option>' + '<option value="ca-3">CA-3</option>' + '<option value="fellow">Fellow</option>' + '</select>' + '</div>' + '<hr /><br />' + '</div>';
}
...
$(".report-milestones-info").popover({
	placement: "left",
	html: "true",
	content: "<ul>" + "<li>Leave empty to include all milestones in training level in report</li>" + "<li>Click a milestone type heading to select all milestones of that type</li>" + "</ul>"
});
...
var options = {
	...
	legendTemplate: "<ul class=\"legend <%=name.toLowerCase()%>-legend\">" + "<% for (var i=0; i<datasets.length; i++){%>" + "<li><span class=\"glyphicon glyphicon-stop\" style=\"color:<%=datasets[i].strokeColor%>\"></span> " + "<%if(datasets[i].label){%><%=datasets[i].label%>" + "<%}%></li><%}%></ul>",
	...
	scaleStepWidth: 2,
	scaleStartValue: 0,
	scaleLabel: function scaleLabel(values) {
...
function drawAverageRadarGraphs() {
	reportData.forEach(function (report, index) {
		...
		var milestoneData = {
			...
			datasets: [report.averageMilestoneDataset]
		};

		var competencyData = {
			labels: report.competencyLabels,
			datasets: [report.averageCompetencyDataset]
		};
...
function prepareReport(report) {
	...
	report.subjectIds = _.keys(report.subjects).sort(function (a, b) {
		return report.subjects[a].localeCompare(report.subjects[b]);
	});

	if (report.trainingLevel == "fellow") report.scaleLabels = ["", "Fellow Level 1", "Fellow Level 2", "Fellow Level 3", "Fellow Level 4", "Fellow Level 5"];else report.scaleLabels = ["", "CBY", "CA-1", "CA-2", "CA-3", "Attending"];
...
function drawLineChart(canvas, chartData, chartLabels) {
	if (evalHistoryChart[canvas] != undefined) evalHistoryChart[canvas].destroy();
...
function getChartEvalData(evals, startDate, endDate, increment, incrementNum) {
	...
	if (labelFormats[increment] != undefined) labelFormat = labelFormats[increment];
	var labels = [];
	var chartData = {
		Requested: [],
		Completed: []
	...
	for (var start = startDate; start < endDate; start.add(incrementNum, increment)) {
		...
		var r = evals.reduce(function (num, e) {
			if (e.request_date != undefined) {
				...
				if (rd >= start && rd < end) return num + 1;
			}
			return num;
		}, 0);
		...
		var c = evals.reduce(function (num, e) {
			if (e.complete_date != undefined && e.status == "complete") {
				...
				if (cd >= start && cd < end) return num + 1;
			}
			return num;
		}, 0);
